...
public class IndoorController {
    ...
    @GetMapping(value = "monthParamContrast/{proId}/{type}/{coding}/{year}/{month}/{indoorId}")
    public Result monthParamContrast(@PathVariable String proId, @PathVariable int type, @PathVariable String coding, @PathVariable int year, @PathVariable int month, @PathVariable String indoorId) {
        return Result.result(1, indoorService.MonthParamContrast(proId, type, coding, year, month, indoorId));
    }
}
